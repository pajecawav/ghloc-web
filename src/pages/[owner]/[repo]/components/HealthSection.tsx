...
export const HealthSection = async ({ owner, repo, data }: HealthSectionProps) => {
	...
	let health;
	try {
		health = await withServerTiming(event, "heatlh", () => ghApi.getRepoHealth(owner, repo));
	} catch (error) {
		console.error(error);
	...
	if (!issue) {
		issue = await withServerTiming(event, "issue_template", () =>
			ghApi.getIssueTemplate(owner, repo),
		);
	}
